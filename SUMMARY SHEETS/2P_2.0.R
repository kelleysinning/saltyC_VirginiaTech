...
SECPROD_EAS <- function(SECPROD, site_filter = "EAS") {
  SECPROD %>%
    ...
    summarise(
      ...
      Mean.Individual.Mass = mean(Individual.Mass, na.rm = TRUE) # Average Individual Mass..avg reps together for each sample month 
    ) %>%
...
COREPROD_sum_summary <- COREPROD_sum %>%
  group_by(Site) %>%
  summarise(Sum.Annual.Production = sum(Sum.Annual.Production))
