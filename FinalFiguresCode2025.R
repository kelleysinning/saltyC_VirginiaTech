...
glm_model.fig4 <- TOTALPROD_Summary_Sum %>%
  ...
  mutate(
    # Extract p-values for slope term
    p_value = map_dbl(glm_model, ~ summary(.x)$coefficients[2, 4]),
    
    # Calculate pseudo-R²
    pseudo_r2 = map_dbl(glm_model, ~ 1 - (.x$deviance / .x$null.deviance)),
    
    # Calculate AIC
    AIC_value = map_dbl(glm_model, AIC),
    
    # Extract null deviance
    null_deviance = map_dbl(glm_model, ~ summary(.x)$null.deviance),
    
    # Extract residual deviance
    residual_deviance = map_dbl(glm_model, ~ summary(.x)$deviance),
    
    # Extract tidy coefficients table
    coef_table = map(glm_model, broom::tidy)
  ) %>%
  unnest(coef_table) %>%  # Expand coefficient table into rows
  select(
    FFG, term, estimate, std.error, statistic, p.value,
    p_value, pseudo_r2, AIC_value, null_deviance, residual_deviance
  ) %>%
  mutate(
...
glm_model.fig7 <- prod.food.ffg %>%
  ...
  mutate(
    ...
    residual_deviance = map_dbl(glm_model, ~ summary(.x)$deviance),
    p_value = map_dbl(glm_model, ~ summary(.x)$coefficients[2, 4]),
    pseudo_r2 = map_dbl(glm_model, ~ 1 - (.$deviance / .$null.deviance)),
...
# BIOMASS SEASONAL NMDS (FIGURE 2)----------------------------------------------------
